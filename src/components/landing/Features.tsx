...
const FEATURES: FeatureDetail[] = [
  ...
  },
  {
    title:   'Connectors',
    badge:   'Integrations',
    tagline: 'Sync data from Google Drive, OneDrive, Dropbox, databases and REST APIs.',
    ...
    desc:    'Sync data from Google Drive, OneDrive, Dropbox, databases and REST APIs.',
    videoId: null,
    ...
    benefits: [
      ...
      { title: 'Unified view',      icon: 'M4 6h16M4 10h16M4 14h16M4 18h16', desc: 'All synced assets appear in a single browsable library alongside your native files.' },
      { title: 'Error alerts',      icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', desc: 'If a connector fails or times out, admins are alerted immediately via email and in-app notification.' },
    ],
  },
  {
  ...
  },
  {
    ...
    badge:   'Security',
    tagline: 'Audit logs, 2FA, ACL, data classification and role-based access control.',
    ...
    desc:    'Audit logs, 2FA, ACL, data classification and role-based access control.',
    videoId: null,
    ...
    benefits: [
      ...
      { title: 'Immutable audit log',  icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', desc: 'Every action is logged with actor, IP, and timestamp — export to CSV for compliance.' },
  ...
  },
  {
    title:   'Analytics',
    badge:   'Analytics',
    tagline: 'Storage usage, access reports, share analytics and activity dashboards.',
    ...
    desc:    'Storage usage, access reports, share analytics and activity dashboards.',
    videoId: null,
    videoTitle: 'Analytics Demo',
    description:
      'Track storage growth per workspace and per user, see which files are accessed most, monitor share-link engagement by view count and geography, and run full audit reports filtered by user, action, date range, or resource type. All charts are exportable as CSV or PNG.',
    benefits: [
      { title: 'Storage breakdown',   icon: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4', desc: 'Visualise storage used by workspace, user, file type, and date range.' },
      { title: 'Share analytics',      icon: 'M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z', desc: 'View counts, download counts, referrer sources, and viewer countries for every shared link.' },
      { title: 'Access patterns',      icon: 'M13 10V3L4 14h7v7l9-11h-7z', desc: 'Find your most-accessed files and least-active workspaces to optimise storage allocation.' },
      { title: 'CSV / PNG export',     icon: 'M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4', desc: 'Export any report as a CSV for spreadsheet processing or as a PNG for stakeholder presentations.' },
    ],
  },
  {
    title:   'API & Developer Tools',
    badge:   'Developer',
    tagline: 'Full REST API, webhooks and SDKs to build on top of your file infrastructure.',
    color:   'text-teal-600 bg-teal-50 border-teal-200',
    icon:    icon('M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4'),
    desc:    'Full REST API, webhooks and SDKs to build on top of your file infrastructure.',
    videoId: null,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f) => (
            <button
              ...
              type="button"
              onClick={() => setActive(f)}
              className="group text-left p-5 bg-white border border-zinc-200 rounded-2xl hover:bg-zinc-950 hover:border-zinc-950 hover:shadow-xl transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-red active:scale-[0.98]"
            >
              {/* Icon */}
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center border mb-4 transition-transform duration-200 group-hover:scale-110 ${f.color}`}>
                {f.icon}
              </div>

              {/* Title */}
              <h3 className="font-bold text-brand-red mb-2 group-hover:text-white transition-colors duration-150 text-sm">
                {f.title}
              </h3>

              {/* Description */}
              <p className="text-xs text-zinc-500 group-hover:text-zinc-400 leading-relaxed transition-colors duration-150">
                {f.desc}
              </p>

              {/* Arrow nudge on hover */}
              <div className="flex items-center gap-1 mt-4 text-[11px] font-semibold text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
