...
/** Converts Meilisearch <em>…</em> highlight markup to React nodes safely — no innerHTML. */
function safeHighlight(raw: string): React.ReactNode {
                ...
                {filtered.map((hit, i) => (
                  <div key={hit.id} className={cn('flex items-start gap-4 px-5 py-4 hover:bg-brand-white-soft dark:hover:bg-dark-surface-2 transition-colors cursor-pointer group', i > 0 && 'border-t border-brand-gray dark:border-dark-border')}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {hit.highlight && (
                        <p className="text-xs text-brand-gray-dark dark:text-dark-text-muted line-clamp-2 [&_mark]:bg-amber-200 dark:[&_mark]:bg-amber-900 [&_mark]:rounded [&_mark]:px-0.5 [&_mark]:text-brand-black dark:[&_mark]:text-dark-text">
                          {safeHighlight(hit.highlight)}
                        </p>
                      )}
                    </div>
