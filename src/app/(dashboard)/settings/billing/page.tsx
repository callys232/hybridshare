...
function PlanCard({ plan, isAnnual, onSelect, isLoading }: {
  plan: typeof PLANS[0];
  isAnnual: boolean;
  ...
  return (
    <div className={cn(
      ...
      <button
        ...
        className={cn(
          'w-full py-2.5 rounded-xl font-bold text-sm transition-all duration-150 active:scale-[0.98] flex items-center justify-center gap-2',
          plan.current
          ...
          !plan.current && !isLoading && `border-current text-brand-black`,
          isLoading && 'opacity-60 cursor-wait'
        )}
        style={!plan.current && !isLoading ? { borderColor: plan.color, color: plan.color } : undefined}
      >
        {isLoading && <span className="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />}
        {plan.current ? '✓ Current Plan' : plan.price === 0 ? 'Downgrade' : isLoading ? 'Redirecting…' : 'Upgrade'}
      </button>
    </div>
  );
}
    ...
    <div className="flex items-center gap-4 px-5 py-3.5 hover:bg-brand-white-soft transition-all duration-150 group border-b border-brand-gray last:border-0">
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-brand-black">
          {invoice.plan} Plan &mdash; {new Date(invoice.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </p>
...
export default function BillingPage() {
  ...
  useEffect(() => {
    api.get<ApiResponse<{ invoices: Invoice[]; billing: BillingInfo }>>('/payments/billing')
      .then((r) => {
        if (r.data.data) {
          setInvoices(r.data.data.invoices ?? []);
          setBillingInfo(r.data.data.billing ?? null);
        }
      })
      .catch(() => {
        // Show static fallback in dev
  ...
  const handleSelectPlan = async (planId: string) => {
    if (planId === 'enterprise') {
      window.open('mailto:sales@lamidgroup.com?subject=Enterprise Plan Inquiry', '_blank');
      return;
    }
    setCheckoutError(null);
    setIsCheckoutLoading(planId);
    try {
      const res = await api.post<ApiResponse<{ url: string }>>('/payments/checkout', {
        planId,
  ...
  const handleCancelPlan = async () => {
    if (!confirm('Are you sure you want to cancel your plan? You will keep access until the end of your billing period.')) return;
    setIsCancelLoading(true);
    try {
      await api.post('/payments/cancel');
      window.location.reload();
    } catch {
      alert('Could not cancel plan. Please contact support.');
    } finally {
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mb-2">
              <h2 className="text-2xl font-black">{billingInfo?.planType ?? user?.planType ?? 'Free'}</h2>
            ...
            {billingInfo?.renewsAt && (
              <p className="text-zinc-300 text-sm">
                ${billingInfo.amount}/month &middot; Renews {new Date(billingInfo.renewsAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            )}
          </div>
          {billingInfo && (
            <div className="text-right">
              ...
              <p className="text-xl font-black text-white">${billingInfo.amount}.00</p>
              <p className="text-xs text-zinc-400">{new Date(billingInfo.renewsAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
            </div>
          )}
        </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between text-xs mb-1">
              ...
              <span className="text-white font-semibold">
                {billingInfo
                  ? `${(billingInfo.storageUsed / 1e9).toFixed(1)} GB / ${(billingInfo.storageQuota / 1e9).toFixed(0)} GB`
                  : `${((user?.storageUsed ?? 0) / 1e9).toFixed(1)} GB`}
              </span>
            </div>
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div
                className="h-full bg-white/60 rounded-full"
                style={{ width: `${Math.min(100, ((billingInfo?.storageUsed ?? user?.storageUsed ?? 0) / (billingInfo?.storageQuota ?? user?.storageQuota ?? 1)) * 100).toFixed(1)}%` }}
              />
            </div>
          </div>
          <button
            ...
            className="text-xs font-semibold text-white bg-white/10 hover:bg-red-500/30 border border-white/20 rounded-lg px-4 py-2 transition-all duration-150 active:scale-[0.98] disabled:opacity-50"
          >
            {isCancelLoading ? 'Cancelling…' : 'Cancel Plan'}
          </button>
        </div>
      </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800 mt-0.5">
                ...
                <a href="mailto:sales@lamidgroup.com" className="font-semibold underline hover:no-underline">Contact sales →</a>
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-700 rounded-2xl p-6 text-white max-w-sm shadow-xl">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-sm font-mono tracking-widest mt-0.5">•••• •••• •••• 4242</p>
              </div>
              <div className="text-right">
