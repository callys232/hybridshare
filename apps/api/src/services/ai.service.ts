...
Only return valid JSON.`;
  ...
  async generateSummary(content: string, createdById: string): Promise<string> {
    const response = await client.messages.create({
      model: this.MODEL,
      max_tokens: 1000,
      messages: [{
...
Return as JSON: { "description": "...", "shortDescription": "..." }`,
  ...
  async generateRecommendations(userId: string) {
    ...
    const [completedCourses, enrolledCourses, userBadges, userProfile] = await Promise.all([
      ...
      prisma.userBadge.findMany({ where: { userId }, take: 5, include: { badge: true } }),
      prisma.user.findUnique({ where: { id: userId }, select: { jobTitle: true, bio: true, xpPoints: true } }),
    ]);
    ...
    const scored = candidates.map((course) => {
      ...
      if (completedCategories.has(course.categoryId)) score += 30;
      const tagMatches = course.tags.filter((t) => completedTags.has(t)).length;
      score += tagMatches * 15;
      return { courseId: course.id, score, reasons: [] as string[] };
    });
  ...
  async chatAssistant(messages: Array<{ role: 'user' | 'assistant'; content: string }>, context?: string) {
...
Be concise, encouraging, and educational. Use markdown formatting for code and lists.`;
  ...
  async analyzeContent(content: string, createdById: string) {
    const response = await client.messages.create({
      model: this.MODEL,
      max_tokens: 1500,
      messages: [{
