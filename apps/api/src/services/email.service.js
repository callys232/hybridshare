...
class EmailService {
    ...
    constructor() {
        this.transporter = nodemailer_1.default.createTransport({
            ...
            tls: { rejectUnauthorized: env_1.env.NODE_ENV === 'production' },
        });
    }
    async send(to, subject, html) {
        try {
            await this.transporter.sendMail({ from: env_1.env.EMAIL_FROM, to, subject, html });
            logger_1.logger.debug('Email sent', { to, subject });
        }
        catch (error) {
            logger_1.logger.error('Email send failed', { to, subject, error });
            throw error;
        }
    }
    async sendVerificationEmail(email, name, token) {
        const url = `${env_1.env.APP_URL}/auth/verify-email?token=${token}`;
        await this.send(email, 'Verify your HybridShare account', this.verificationTemplate(name, url));
    }
    async sendPasswordResetEmail(email, name, token) {
        const url = `${env_1.env.APP_URL}/auth/reset-password?token=${token}`;
        await this.send(email, 'Reset your HybridShare password', this.passwordResetTemplate(name, url));
    }
    async sendShareNotification(email, senderName, resourceName, shareUrl) {
        await this.send(email, `${senderName} shared "${resourceName}" with you`, this.shareNotificationTemplate(senderName, resourceName, shareUrl));
    }
    async sendWorkspaceInvite(email, inviterName, workspaceName, inviteUrl) {
        await this.send(email, `You've been invited to join ${workspaceName} on HybridShare`, this.workspaceInviteTemplate(inviterName, workspaceName, inviteUrl));
    }
    async sendWeeklyDigest(email, name, stats) {
...
<body style="margin:0;padding:0;background-color:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    shareNotificationTemplate(sender, resource, url) {
        return this.baseTemplate(`
    ...
    workspaceInviteTemplate(inviter, workspace, url) {
        return this.baseTemplate(`
