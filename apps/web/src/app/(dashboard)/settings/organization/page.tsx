  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-zinc-400 text-sm mt-1">Manage your team, integrations, and enterprise settings</p>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-zinc-800 mb-8 gap-0 overflow-x-auto">
        {ORG_TABS.map((tab) => (
          <button
            key={tab}
            type="button"
            ...
            className={cn(
              ...
              activeTab === tab ? 'border-brand-red text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300 hover:border-zinc-700'
            )}
          >
            {tab}
          </button>
      ...
      {activeTab === 'General' && (
        <div className="space-y-5">
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Organization Name</label>
            <input
              type="text"
              value={orgForm.name}
              onChange={(e) => setOrgForm((f) => ({ ...f, name: e.target.value }))}
              className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-brand-red transition-colors"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Primary Domain</label>
            <input
              type="text"
              ...
              className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-brand-red transition-colors"
            />
            <p className="text-xs text-zinc-500 mt-1">Used to auto-assign new users to your organization</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            <p className="text-xs text-zinc-400">
              <span className="text-white font-semibold">Plan: </span>{organization?.plan ?? 'FREE'} Â·
        ...
        <div className="space-y-5">
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:border-brand-red transition-colors"
            />
            <select
          ...
          <div className="space-y-2">
            {members.map((member) => (
              <div key={member.id} className="flex items-center justify-between p-3 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-all duration-150">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {member.role !== 'OWNER' && (
                    <button
                      ...
                      className="text-zinc-600 hover:text-brand-red transition-colors p-1 rounded hover:bg-zinc-800"
                    >
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">Allow members to log in with your identity provider</p>
            </div>
            <button
              type="button"
              ...
              className={cn(
                ...
                ssoForm.isEnabled ? 'bg-brand-red' : 'bg-zinc-700'
              )}
            >
          ...
          {ssoForm.isEnabled && (
            <>
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Provider</label>
                  ...
                  {['SAML', 'OIDC', 'LDAP'].map((p) => (
                    <button
                      ...
                      className={cn(
                        ...
                        ssoForm.provider === p ? 'bg-brand-red text-white border-brand-red' : 'bg-zinc-900 text-zinc-400 border-zinc-700 hover:border-zinc-600 hover:text-white'
                      )}
                    >
                      {p}
                    </button>
              ...
              {ssoForm.provider === 'SAML' && (
                <div>
                  <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Identity Provider Metadata URL</label>
                  <input
                    type="url"
              ...
              {(ssoForm.provider === 'OIDC') && (
                <>
                  <div>
                    <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Client ID</label>
                    <input
                      type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Allowed Domain</label>
                <input
                  type="text"
      ...
      {activeTab === 'Branding' && (
        <div className="space-y-5">
          <div className="bg-zinc-900 border border-amber-500/20 rounded-xl p-4 text-xs text-amber-400">
            White-labeling is available on the Enterprise plan.
          </div>
          <div>
            <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Primary Color</label>
            <div className="flex items-center gap-3">
              <input
                type="color"
              ...
              <input
                ...
                value={brandingForm.primaryColor}
                onChange={(e) => setBrandingForm((f) => ({ ...f, primaryColor: e.target.value }))}
                className="bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-brand-red transition-colors w-32"
              />
            </div>
          </div>
          {[
      ...
      {activeTab === 'Webhooks' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-zinc-400">{webhooks.length} endpoint{webhooks.length !== 1 ? 's' : ''} configured</p>
            <button
              type="button"
          ...
          {showWebhookForm && (
            <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 space-y-4 animate-fade-in">
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">Endpoint URL</label>
                <input
                  type="url"
                  value={webhookForm.url}
                  onChange={(e) => setWebhookForm((f) => ({ ...f, url: e.target.value }))}
                  placeholder="https://your-server.com/webhooks/hybridshare"
                  className="w-full bg-zinc-950 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:border-brand-red transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-2">Events to Subscribe</label>
                <div className="grid grid-cols-2 gap-2">
                  {WEBHOOK_EVENTS.map((evt) => (
                    <label key={evt.value} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      <span className="text-xs text-zinc-300 group-hover:text-white transition-colors">{evt.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowWebhookForm(false)} className="flex-1 py-2 bg-zinc-800 text-sm text-white rounded-lg hover:bg-zinc-700 transition-all font-medium">Cancel</button>
          ...
          {webhooks.map((wh) => (
            <div key={wh.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 hover:border-zinc-700 transition-all duration-200">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <div className={cn('w-2 h-2 rounded-full', wh.isActive ? 'bg-green-400' : 'bg-zinc-600')} />
                    <code className="text-xs text-zinc-300 font-mono truncate">{wh.url}</code>
                  </div>
                ...
                <button
                  ...
                  className="p-1.5 text-zinc-600 hover:text-brand-red hover:bg-zinc-800 rounded transition-all shrink-0"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
